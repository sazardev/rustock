...
Pedido del usuario: "crea un mini script para que el futuro sea todo levantarlo
fácilmente con un solo comando". Se creó un wrapper bash que encapsula todo lo
que antes había que hacer a mano (limpiar procesos viejos en los puertos,
elegir base de datos, seedear) y delega el arranque real al orquestador
existente `scripts/web.mjs` (Hito 11) — no se duplica la lógica de spawn.

- **`scripts/dev.sh` (nuevo, ejecutable)**: detecta y mata cualquier proceso
  que ocupe `:6821` o `:1421` antes de arrancar (vía `ss` + `kill`), prepara
  la base según la opción y `exec`s `npm run tauri:web`. Opciones:
  - `--seed` → `RUSTOCK_SEED=1` (datos de ejemplo si la base está vacía).
  - `--reset` → respalda la db real (`.backup-<timestamp>`) y la borra, luego
    seedea.
  - `--tmpdb` → usa `RUSTOCK_DB_PATH=/tmp/opencode/rustock-dev.db`, no toca la
    real. Combinable con `--seed`.
  - `--stop` → mata cualquier instancia en los puertos (sin levantar nada).
  - `--help` → imprime el uso (autoextraído de los comentarios del header).
  - Respeta `RUSTOCK_HTTP_PORT` para el API y `RUSTOCK_DB_PATH` para la base.
- **`package.json`**: script npm nuevo `"dev:web": "bash scripts/dev.sh"`.
- **AGENTS.md**: línea del comando `dev:web` en la tabla de comandos.
- **Sin lógica de negocio**: es puramente orquestación de arranque; toda la
  lógica de datos sigue en Rust.

**Verificación en vivo:** `--help` imprime el uso; `--stop` detuvo la instancia
que corría (mató vite `:6821` y el backend `:1421` quedó libre); arranque con
`--seed` desde cero (cargo ya compilado → ~9 s): vite `:6821` HTTP 200 + backend
`:1421` con la db que ya tenía seed (aviso idempotente "[seed] ya hay datos, no
se siembra de nuevo") y dashboard respondiendo. Preferir `./scripts/dev.sh
--seed` (o `npm run dev:web -- --seed`) para el día a día en vez de
`RUSTOCK_SEED=1 npm run tauri:web`.

**Nota de concurrencia:** `pkill -f 'tauri:web'` se auto-mató a sí mismo (el
patrón coincidía con su propia línea de comando) al detener la instancia
anterior — por eso `dev.sh` mató por puerto (regex acotada a `:<puerto>\b` con
`rg` + extracción del pid), no por nombre de proceso. No repetir `pkill -f` con
patrones que coincidan con el propio comando.
...
**Script de arranque en un solo comando (`dev.sh` / `npm run dev:web`,
Hito 26, §2, sin commit)**: wrapper bash que limpia los puertos `:6821`/
`:1421` de instancias viejas, prepara la base según la opción
(`--seed`/`--reset`/`--tmpdb`/`--stop`/`--help`) y delega el arranque real a
`scripts/web.mjs` (no duplica la lógica de spawn). Para el día a día:
